  ...
  *(void**)(&hdfsSeek) = dlsym(dl, "hdfsSeek");
  ...
  if (error != NULL) {
	...
	_libraryInstance = ::LoadLibraryW(L"hdfs.dll");
		...
		LocalFree( str );
    		fprintf(stderr, "resolve symbol from hdfs.dll\n");
        }
...
int32_t Hdfs::CreateDirectoryHdfs(const std::string& path) {
  tera::AutoCounter ac(&dfs_other_hang_counter, "CreateDirectoryHdfs", path.c_str());
  dfs_other_counter.Inc();
  return (*hdfsCreateDirectoryHdfs)((hdfsFS)fs_, path.c_str());
}
