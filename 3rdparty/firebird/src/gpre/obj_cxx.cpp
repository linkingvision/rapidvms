...
%ENwhile (1)\n\
   {";
	...
	column += INDENT;
	gen_get_segment(action, column);
	printa(column, "if (fbIStatus == Firebird::IStatus::RESULT_ERROR || "
				   "fbIStatus == Firebird::IStatus::RESULT_NO_DATA) break;");
}
...
static void gen_function( const act* function, int column)
{
	...
	fprintf(gpreGlob.out_file, "static %s_r (Firebird::IRequest* request, Firebird::ITransaction* transaction ", request->req_handle);
...
static void gen_get_segment( const act* action, int column)
{
	...
	if (action->act_flags & ACT_sql)
	{
		...
		set_sqlcode(action, column);
		printa(column, "if (fbIStatus == Firebird::IStatus::RESULT_OK || fbIStatus == Firebird::IStatus::RESULT_SEGMENT)");
		column += INDENT;
		begin(column);
		align(column);
		fprintf(gpreGlob.out_file, "isc_ftof (fb_%d, fb_%d, %s, fb_%d);",
				   blob->blb_buff_ident, blob->blb_len_ident,
				   into->ref_value, blob->blb_len_ident);
		if (into->ref_null_value)
		{
			align(column);
			fprintf(gpreGlob.out_file, "%s = fb_%d;", into->ref_null_value, blob->blb_len_ident);
		}
		endp(column);
		column -= INDENT;
	}
...
static TEXT* gen_name(char* const string, const ref* reference, bool as_blob)
{
	if (reference->ref_field->fld_array_info && !as_blob)
		fb_utils::snprintf(string, MAX_REF_SIZE, "fb_%d",
				reference->ref_field->fld_array_info->ary_ident);
	else if (reference->ref_port)
		fb_utils::snprintf(string, MAX_REF_SIZE, "fb_%d.fb_%d",
				reference->ref_port->por_ident, reference->ref_ident);
	else
		fb_utils::snprintf(string, MAX_REF_SIZE, "fb_%d", reference->ref_ident);
...
static void gen_put_segment( const act* action, int column)
{
	...
	if (action->act_flags & ACT_sql)
	{
		...
		align(column);
		fprintf(gpreGlob.out_file, "fb_%d = %s;", blob->blb_len_ident, from->ref_null_value);
		align(column);
		fprintf(gpreGlob.out_file, "isc_ftof (%s, fb_%d, fb_%d, fb_%d);",
...
static void gen_receive( const act* action, int column, const gpre_port* port)
{
	PAT args;
	const TEXT* pattern =
		"%RH->receive (%V1, %RL, %PN, %PL, CAST_MSG(&%PI));";
...
static void gen_request(const gpre_req* request)
{
	...
	{
		printa(0, "static Firebird::IRequest*\n   %s = 0;\t\t/* request handle */\n", request->req_handle);
	}
	...
	if (request->req_flags & (REQ_sql_blob_open | REQ_sql_blob_create))
		printa(0, "static Firebird::IStatement*\n   fb_%ds;\t\t/* sql statement handle */",
			   request->req_ident);
	...
	if (request->req_length)
	{
		...
		printa(INDENT, "};\t/* end of %s string for request fb_%d */\n",
	...
	for (const gpre_port* port = request->req_ports; port; port = port->por_next)
	{
		for (const ref* reference = port->por_references; reference; reference = reference->ref_next)
		{
			if (reference->ref_sdl)
			{
				...
				printa(INDENT, "};\t/* end of sdl string for request fb_%d */\n",
	...
	for (const blb* blob = request->req_blobs; blob; blob = blob->blb_next)
	{
		...
		{
			printa(0, "static %sunsigned char\n   fb_%d [] = {", CONST_STR, blob->blb_bpb_ident);
	...
	if (request->req_type == REQ_slice)
	{
		...
		printa(INDENT, "fb_%dv [%d],", request->req_ident,
			   MAX(request->req_slice->slc_parameters, 1));
		printa(INDENT, "fb_%ds;", request->req_ident);
	}
}
...
static void gen_routine( const act* action, int column)
{
		...
		request = request->req_routine)
	{
		...
		for (const blb* blob = request->req_blobs; blob; blob = blob->blb_next)
		{
			printa(column, "isc_blob_handle\t\tfb_%d;\t\t/* blob handle */", blob->blb_ident);
			printa(column, "char\t\t\tfb_%d [%d];\t/* blob segment */",
				   blob->blb_buff_ident, blob->blb_seg_length);
			printa(column, "unsigned short\tfb_%d;\t\t/* segment length */", blob->blb_len_ident);
		}
	}
}
...
static void gen_send( const act* action, const gpre_port* port, int column)
{
	PAT args;
	const TEXT* pattern =
		"%RH->send(%V1, %RL, %PN, %PL, CAST_CONST_MSG(&%PI));";
...
(char*) %I1, (short) %N2, %I1v, %I1s, %S5);";
	...
	printa(column, "fb_%ds = %d", request->req_ident, slice->slc_field->fld_array->fld_length);
...
{
	const TEXT* pattern1 =
		"%RH->startAndSend(%V1, %S1, %RL, %PN, %PL, CAST_CONST_MSG(&%PI));";
	const TEXT* pattern2 = "%RH->start(%V1, %S1, %RL);";
...
static void gen_t_start( const act* action, int column)
{
	...
	{
		printa(column, "{\t\t// fbComponents scope\n");
		printa(column + INDENT, "DtcStart fbComponents[%d] = {", trans->tra_db_count);

		for (tpb_iterator = trans->tra_tpb; tpb_iterator; tpb_iterator = tpb_iterator->tpb_tra_next)
		{
			printa(column + INDENT * 2, "{%s, fb_tpb_%d, %d}%s\n",
				tpb_iterator->tpb_database->dbb_name->sym_string,
				tpb_iterator->tpb_ident, tpb_iterator->tpb_length,
				tpb_iterator->tpb_tra_next ? "," : "");
		}
...
static void gen_tpb(const tpb* tpb_buffer, int column)
{
	...
	printa(column, "static %sunsigned char\n", CONST_STR);
	column += INDENT;
	TEXT* p = buffer;
...
static void gen_trans( const act* action, int column)
{
	...
	case ACT_commit_retain_context:
	case ACT_rollback_retain_context:
		printa(column, "%s->%sRetaining (%s);", tranText,
			action->act_type == ACT_commit_retain_context ? "commit" : "rollback",
			status_vector(action));
		break;
	default:
		printa(column, "%s->%s(%s);", tranText,
...
static void make_array_declaration(ref* reference)
{
	...
	fprintf(gpreGlob.out_file, "static %s fb_%d", dtype, field->fld_array_info->ary_ident);
...
{
	printa(column, "struct fb_%d_struct {", port->por_ident);
	...
	for (const ref* reference = port->por_references; reference; reference = reference->ref_next)
	{
		...
		if (fld_len)
			fprintf(gpreGlob.out_file, "    %s fb_%d [%d];\t/* %s */",
					   dtype, reference->ref_ident, fld_len, name);
		else
			fprintf(gpreGlob.out_file, "    %s fb_%d;\t/* %s */", dtype, reference->ref_ident, name);
	}

	printa(column, "} fb_%d;", port->por_ident);
}
					   ...
					   const gpre_req* request)
{
	...
	align(column);
	if (filename)
		fprintf(gpreGlob.out_file, "%s = fbProvider->attachDatabase(%s, %s, %s, %s);",
				   db->dbb_name->sym_string, vector, filename,
				   (request ? s1 : dpb_size_ptr), (request ? s2 : dpb_ptr));
	else
		fprintf(gpreGlob.out_file,
				   "%s = fbProvider->attachDatabase(%s, \"%s\", %s, %s);",
				   db->dbb_name->sym_string, vector, db->dbb_filename,
				   (request ? s1 : dpb_size_ptr), (request ? s2 : dpb_ptr));
						 ...
						 bool test)
{
	...
	if (count == 1)
	{
		printa(column, "%s = %s->startTransaction(%s, 0, NULL);",
			trname, gpreGlob.isc_databases->dbb_name->sym_string, vector);
	}
	else
	{
		printa(column, "{\t\t// fbComponents scope\n");
		printa(column + INDENT, "DtcStart fbComponents[%d] = {", count);
