...
static int dxva2_alloc(AVCodecContext *s)
{
    ...
    if (!ctx->dxva2lib) {
		...
		printf("Failed to load DXVA2 library\n");
        goto fail;
    }
...
#else
    if (ist->hwaccel_get_d3d_dev)
    {
        ctx->d3d9device = ist->hwaccel_get_d3d_dev(s, ist->hwaccel_cb_ctx);
        if (ctx->d3d9device == NULL)
        {
           av_log(NULL, loglevel, "Failed to get Direct3D device\n");
           goto fail;
        }
    }else
    {
