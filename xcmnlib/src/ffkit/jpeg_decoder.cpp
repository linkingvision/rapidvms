...
jpeg_decoder::jpeg_decoder( const struct codec_options& options, int decodeAttempts ) :
    _codec( avcodec_find_decoder( AV_CODEC_ID_MJPEG ) ),
...
{
    if( !_codec )
        CK_THROW(( "Failed to find jpeg decoder." ));
    ...
    if( avcodec_open2( _context, _codec, NULL ) < 0 )
        CK_THROW(( "Unable to open jpeg decoder." ));
}

jpeg_decoder::jpeg_decoder( av_demuxer& deMuxer, const struct codec_options& options, int decodeAttempts ) :
    _codec( avcodec_find_decoder( AV_CODEC_ID_MJPEG ) ),
    _context( avcodec_alloc_context3( _codec ) ),
    _options( options ),
	_frame(av_frame_alloc()),
...
jpeg_decoder::~jpeg_decoder() throw()
{
    _destroy_scaler();
...
void jpeg_decoder::decode( std::shared_ptr<av_packet> frame )
{
...
uint16_t jpeg_decoder::get_input_width() const
{
    return (uint16_t)_context->width;
}

uint16_t jpeg_decoder::get_input_height() const
{
    return (uint16_t)_context->height;
}

void jpeg_decoder::set_output_width( uint16_t outputWidth )
{
...
uint16_t jpeg_decoder::get_output_width() const
{
    return _outputWidth;
}

void jpeg_decoder::set_output_height( uint16_t outputHeight )
{
...
uint16_t jpeg_decoder::get_output_height() const
{
    return _outputHeight;
}

shared_ptr<av_packet> jpeg_decoder::get()
{
    if( _outputWidth == 0 )
...
void jpeg_decoder::_destroy_scaler()
{
    if( _scaler )
    {
