...
#define TRUE_OR_RETURN(a)                                            \
  do {                                                               \
	...
	static_assert(sizeof(kTableSarWidth) == sizeof(kTableSarHeight),
	...
	H264Parser::Result H264Parser::ParseSPSScalingLists(H264SPS* sps) {
		...
		for (int i = 0; i < 6; ++i) {
			...
			if (seq_scaling_list_present_flag) {
				res = ParseScalingList(sizeof(sps->scaling_list4x4[i]) / sizeof(sps->scaling_list4x4[0][0]),
		...
		for (int i = 0; i < ((sps->chroma_format_idc != 3) ? 2 : 6); ++i) {
			...
			if (seq_scaling_list_present_flag) {
				res = ParseScalingList(sizeof(sps->scaling_list8x8[i]) / sizeof(sps->scaling_list8x8[0][0]),
	...
	H264Parser::Result H264Parser::ParsePPSScalingLists(const H264SPS& sps,
		...
		for (int i = 0; i < 6; ++i) {
			...
			if (pic_scaling_list_present_flag) {
				res = ParseScalingList(sizeof(pps->scaling_list4x4[i]) / sizeof(pps->scaling_list4x4[0][0]),
		...
		if (pps->transform_8x8_mode_flag) {
			for (int i = 0; i < ((sps.chroma_format_idc != 3) ? 2 : 6); ++i) {
				...
				if (pic_scaling_list_present_flag) {
					res = ParseScalingList(sizeof(pps->scaling_list8x8[i]) / sizeof(pps->scaling_list8x8[0][0]),
	...
	H264Parser::Result H264Parser::ParseVUIParameters(H264SPS* sps) {
		...
		if (aspect_ratio_info_present_flag) {
			...
			if (aspect_ratio_idc == H264SPS::kExtendedSar) {
				...
				const int max_aspect_ratio_idc = sizeof(kTableSarWidth) / sizeof(kTableSarWidth[0]) - 1;
		...
		*pps_id = -1;
		...
		if (br_.HasMoreRBSPData()) {
			...
			if (pps->pic_scaling_matrix_present_flag) {
				...
				res = ParsePPSScalingLists(*sps, pps);
				if (res != kOk)
					return res;
			}
	...
	H264Parser::Result H264Parser::ParseDecRefPicMarking(H264SliceHeader* shdr) {
		...
		if (shdr->idr_pic_flag) {
			...
			if (shdr->adaptive_ref_pic_marking_mode_flag) {
				size_t i;
				for (i = 0; i < sizeof(shdr->ref_pic_marking) / sizeof(shdr->ref_pic_marking[0]); ++i) {
				...
				if (i == sizeof(shdr->ref_pic_marking) / sizeof(shdr->ref_pic_marking[0])) {
