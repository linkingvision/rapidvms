...
#ifndef __CAP_FFMPEG_IMPL_H__
#define __CAP_FFMPEG_IMPL_H__


#include "ffmpeg_api.hpp"
#include <assert.h>
#include <algorithm>
#include <limits>
...
#define AV_NOPTS_VALUE_ ((s64)0x8000000000000000LL)
#else
#define AV_NOPTS_VALUE_ ((s64)0x8000000000000000LL)
#endif
...
public:
    ...
    bool grabFrame();
    bool grabFrame(unsigned char** dataRGB, int* step, int* width, int* height, int* cn,
            unsigned char** data, int *size);
    bool retrieveFrame(int, unsigned char** data, int* step, int* width, int* height, int* cn);

    void init();

    void    seek(s64 frame_number);
    void    seek(double sec);
    bool    slowSeek( int framenumber );

    s64 get_total_frames();
    double  get_duration_sec();
    double  get_fps();
    int     get_bitrate();

    double  r2d(AVRational r);
    s64 dts_to_frame_number(s64 dts);
    double  dts_to_sec(s64 dts);
...
private:
    ...
    s64 frame_number, first_frame_number;
...
*/
    ...
    /* Data for av_read_frame */
    unsigned char* pData;
    int nDataSize;
};

inline CvCapture_FFMPEG::CvCapture_FFMPEG()
{
	init();
}

inline void CvCapture_FFMPEG::init()
{
    ic = 0;
    video_stream = -1;
    video_st = 0;
...
inline void CvCapture_FFMPEG::close()
{
    if( img_convert_ctx )
    {
...
inline  void icvInitFFMPEG_internal()
{
...
inline bool CvCapture_FFMPEG::setSize(int w, int h)
{
    if( rgb_picture.data[0] )
    {
        free( rgb_picture.data[0] );
...
inline bool CvCapture_FFMPEG::open( const char* _filename )
{
...
#if LIBAVFORMAT_BUILD >= CALC_FFMPEG_VERSION(52, 111, 0)
    int err = avformat_open_input(&ic, _filename, NULL, &opts);
#else
...
#define AVMEDIA_TYPE_VIDEO CODEC_TYPE_VIDEO
        ...
        if( AVMEDIA_TYPE_VIDEO == enc->codec_type && video_stream < 0) {
            ...
            avpicture_fill( (AVPicture*)&rgb_picture, rgb_picture.data[0],
                            PIX_FMT_RGB24, enc->width, enc->height );
...
void inline DumpHex(char * pBuff, int nSize)
{
...
inline bool CvCapture_FFMPEG::retrieveFrame(int, unsigned char** data, int* step, int* width, int* height, int* cn)
{
...
inline double CvCapture_FFMPEG::r2d(AVRational r)
{
...
inline int CvCapture_FFMPEG::get_bitrate()
{
    return ic->bit_rate;
}
...
inline s64 CvCapture_FFMPEG::dts_to_frame_number(s64 dts)
{
    double sec = dts_to_sec(dts);
    return (s64)(get_fps() * sec + 0.5);
}
...
inline void CvCapture_FFMPEG::seek(double sec)
{
    seek((s64)(sec * get_fps() + 0.5));
}

inline bool CvCapture_FFMPEG::setProperty( int property_id, double value )
{
    ...
    case CV_FFMPEG_CAP_PROP_POS_AVI_RATIO:
        {
            ...
            case CV_FFMPEG_CAP_PROP_POS_AVI_RATIO:
                seek((s64)(value*ic->duration));
                break;
            }

            picture_pts=(s64)value;
        }
        break;
    default:
        return false;
...
inline  const char * icvFFMPEGErrStr(int err)
{
#if LIBAVFORMAT_BUILD >= CALC_FFMPEG_VERSION(53, 2, 0)
    switch(err) {
...
inline void CvVideoWriter_FFMPEG::init()
{
    fmt = 0;
    oc = 0;
    outbuf = 0;
 ...
 */
inline  AVFrame * icv_alloc_picture_FFMPEG(int pix_fmt, int width, int height, bool alloc)
{
    AVFrame * picture;
    uint8_t * picture_buf;
    int size;
...
inline  AVStream *icv_add_video_stream_FFMPEG(AVFormatContext *oc,
                                             CodecID codec_id,
