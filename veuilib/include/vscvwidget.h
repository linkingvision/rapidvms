...
{
	public:
	virtual bool GetHWDec(){return FALSE;}
};
...
public:
	VSCVWidget(ClientFactory &pFactory, s32 nId,  
				QWidget *parent = 0, bool bSingle = false, Qt::WindowFlags flags = 0, 
				VVidGetHWDec *pGetHWDec = NULL);
	~VSCVWidget();
...
public:
	BOOL StartPlay(astring strStorId, astring strCamId, astring strCamName, 
			bool bMotion = false, bool bByView = false, bool bWithHWAccel = false);
	BOOL StopPlay();
	bool GetPlayParam(astring &strStorId, astring &strCamId, bool &bHWAccel);
