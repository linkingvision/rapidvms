...
#define IP_MULTICAST_IF 2
  ...
  if (soap->recv_timeout || soap->send_timeout)
    ...
    SOAP_SOCKBLOCK(fd)
  soap->socket = fd;
/* xsmart get local ip addr */
  soap->locallen = sizeof(soap->local);
  memset(&soap->local, 0, soap->locallen);
#ifdef WIN32
#include <winsock.h>
#endif
#ifdef WIN32
  getsockname(soap->socket, (sockaddr *)&soap->local, (int *)&(soap->locallen));
#else
  getsockname(soap->socket, (sockaddr *)&soap->local, (socklen_t *)&(soap->locallen));
#endif
...
/* Use soap->host to store the numeric form of the remote host */
    ...
    snprintf
#endif
     (soap->host, sizeof(soap->host), "%u.%u.%u.%u", (int)(soap->ip>>24)&0xFF, (int)(soap->ip>>16)&0xFF, (int)(soap->ip>>8)&0xFF, (int)soap->ip&0xFF);
#else
