...
{ @autoreleasepool
{
    ...
    data->urhoPlaceholderView = nsview;
    ...
    if (!data->urhoPlaceholderView)
    {
        ...
        NSRect rect = [data->urhoPlaceholderView bounds];
...
{ @autoreleasepool
{
    ...
    if (SetupWindowData(_this, window, nswindow, NULL, SDL_TRUE) < 0) {
        [nswindow release];
        return -1;
    }
    return 0;
...
{
    NSView * urhoPlaceholderView = (NSView *)data;
@autoreleasepool
{
    NSWindow *nswindow = [urhoPlaceholderView window];
    return SetupWindowData(_this, window, nswindow, urhoPlaceholderView, SDL_FALSE);
}}
...
{
    SDL_WindowData *windata = (SDL_WindowData *) window->driverdata;
    if (windata->urhoPlaceholderView) {
        return;
    }
    NSWindow *nswindow = windata->nswindow;
    NSRect rect;
